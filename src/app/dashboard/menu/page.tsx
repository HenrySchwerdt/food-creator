      ...
      <div className="flex flex-row gap-4">
        <div className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-2">
            {today != "Mon" && (
              <MenuDayView menu={menuWeek!.mon} day="Montag" />
            )}
            {today != "Tue" && (
              <MenuDayView menu={menuWeek!.tue} day="Dienstag" />
            )}
            {today != "Wed" && (
              <MenuDayView menu={menuWeek!.wen} day="Mittwoch" />
            )}
            {today != "Thu" && (
              <MenuDayView menu={menuWeek!.thu} day="Donnerstag" />
            )}
            {today != "Fri" && (
              <MenuDayView menu={menuWeek!.fri} day="Freitag" />
            )}
            {today != "Sat" && (
              <MenuDayView menu={menuWeek!.sat} day="Samstag" />
            )}
            {today != "Sun" && (
              <MenuDayView menu={menuWeek!.sun} day="Sonntag" />
            )}
          </div>
        </div>
