...
function extractProductAldiData(): ProductData[] {
  ...
  productElements.forEach((product) => {
    const imgUrl =
      product.querySelector("img")!.getAttribute("data-src") ?? null;
    const discount =
      (product.querySelector(
        "figcaption p b",
      ) as HTMLElement).innerText.trim() ?? null;
    ...
    const additionalInfo =
      (product.querySelector(
        'figcaption p[style*="font-size: 11.0px;"]',
      ) as HTMLElement)!.innerText!.trim() ?? null;
...
function extractProductLidlData(): ProductData[] {
  ...
  productElements.forEach((productElement) => {
    const productData: ProductData = {
      ...
      price:
        (
          productElement.querySelector(".m-price__price") as HTMLElement
        )!.innerText.trim() ?? null,
      originalPrice:
        (
          productElement.querySelector(
      ...
      discount:
        (
          productElement.querySelector(".m-price__label") as HTMLElement
        )!.innerText.trim() ?? null,
      description:
        (
          productElement.querySelector(".product-grid-box__desc") as HTMLElement
        )!.innerText.trim() ?? null,
      img:
        (
          productElement.querySelector(
      ...
      packaging:
        (productElement.querySelector(".price-footer") as HTMLElement)!.innerText
          .trim()
          .replace(/[\n\r]+/g, " ") ?? null,
      availability:
        (
          productElement.querySelector(".ods-badge__label") as HTMLElement
        )!.innerText.trim() ?? null,
      dataOrigin: "LIDL",
    };
