...
function extractProductAldiData() {
    ...
    productElements.forEach(product => {
        ...
        const discount = product.querySelector('figcaption p b')?.innerText.trim() || null;
        const priceElement = product.querySelector('figcaption p[style*="font-size: 25.0px;"]');
        ...
        const additionalInfo = product.querySelector('figcaption p[style*="font-size: 11.0px;"]')?.innerText.trim() || null;
...
function extractProductLidlData() {
    ...
    productElements.forEach(productElement => {
        ...
        // Extract Image URL
        const imageElement = productElement.querySelector('.ods-image-gallery__image');
        productData.img = imageElement ? imageElement.src : null;

        // Extract Product Name
        const productNameElement = productElement.querySelector('.product-grid-box__title');
        productData.name = productNameElement ? productNameElement.innerText.trim() : null;

        // Extract Product Description
        const productDescElement = productElement.querySelector('.product-grid-box__desc');
        productData.description = productDescElement ? productDescElement.innerText.trim() : null;

        // Extract Discount (if any)
        const discountElement = productElement.querySelector('.m-price__label');
        productData.discount = discountElement ? discountElement.innerText.trim() : null;

        // Extract Original Price
        const originalPriceElement = productElement.querySelector('.strikethrough.m-price__rrp');
        productData.originalPrice = originalPriceElement ? originalPriceElement.innerText.trim() : null;

        // Extract Current Price
        const currentPriceElement = productElement.querySelector('.m-price__price');
        productData.price = currentPriceElement ? currentPriceElement.innerText.trim() : null;

        // Extract Packaging Information
        const packagingElement = productElement.querySelector('.price-footer');
        productData.packaging = packagingElement ? packagingElement.innerText.trim().replace(/[\n\r]+/g, ' ') : null;

        // Extract Availability Information
        const availabilityElement = productElement.querySelector('.ods-badge__label');
        productData.availability = availabilityElement ? availabilityElement.innerText.trim() : null;
